...
def speed(gamestates):
    ...
    """
    #each iteration is a game - i think?!!!
    a0 = gamestates[0]
    print(gamestates)
    ...
    return speed
@required_fields(["freeze_frame_360", "time_seconds"])
def player_speed(gamestates):
    """Get the speed at which all players moved
    ...
    -------
    Features
        A dataframe with columns 'player_speed_a0' describing the speed
        of all players in m/s  between action a0 and action a1. """
    #print(gamestates)
    a0 = gamestates[0]
    a1 = gamestates[1]
    time_col = a0.time_seconds - a1.time_seconds
    time_col[time_col <= 0] = 1e-6
    def get_speeds(a0_dict, a1_dict, timediff):
        if(not isinstance(a0_dict, list) or not isinstance(a1_dict, list)):
            return None
        output = []
        a0locs = [[p['x'], p['y']] for p in a0_dict]
        for player in a1_dict:
            playerdict = {}
        ...
        return output
    speeddf = pd.DataFrame(data = {"a0": a0.freeze_frame_360, "a1": a1.freeze_frame_360, "timediff": time_col}, index = a0.index)
    speeddf.index = a0.index
    speeds = speeddf.apply(lambda x: get_speeds(*x), axis=1).rename("player_speeds_a01").to_frame()
    print(speeds)
    return speeds
...
def simulate_features(
    ...
    if action_id is not None:
        actions = add_names(db.actions(game_id).loc[action_id])
    # filter actions of interest
    if actionfilter is None:
        idx = pd.Series([True] * len(actions), index=actions.index)
    else:
