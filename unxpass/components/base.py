...
class UnxPassXGBoostComponent(UnxpassComponent):
    ...
    def predict_locations(self, dataset, game_id, db, xy_coo, action_id = None, result=None) -> pd.Series:
        data = self.initialize_dataset(dataset)
    ...
    def predict_surface(
        ...
        out = {}
        firstteam = db.games().loc[game_id]["home_team_id"]
        game_actions = db.actions(game_id = game_id)
        cols = [item for sublist in data.xfns.values() for item in sublist]
        for action_id in sim_features.index.unique(level=1):
            team = game_actions.loc[(game_id, action_id)]["team_id"]
            if isinstance(self.model, xgb.XGBClassifier):
                ...
                raise AttributeError(f"Unsupported xgboost model: {type(self.model)}")
            if team != firstteam:
                out[f"action_{action_id}"] = np.flip(out[f"action_{action_id}"])
        return out
...
class UnxPassPytorchComponent(UnxpassComponent):
    ...
    def predict_surface(
        self, dataset, db = None,  game_id=None, subset=None, batch_size=1, num_workers=8, pin_memory=False, **predict_cfg
    ) -> Dict:
        if db is None:
            # Load dataset
            data = self.initialize_dataset(dataset)
            ...
            if subset is not None:
                ...
                actions = actions.iloc[subset].reset_index()
        ...
        predictions = torch.cat(predictor.predict(self.model, dataloaders=dataloader))#concats tensors
        #return predictions
        output = defaultdict(dict)
        ...
        #print(actions)
        #time.sleep(10000)
        for i, action in actions.reset_index(drop = True).iterrows():
            print(f"{iter}/{actions.shape[0]}")
            #print(i)
            iter = iter + 1
            if action.game_id not in firstteam_dict:
                game_actions = db.actions(game_id = action.game_id)
                games_df = db.games().loc[action.game_id]
                #firstteam = game_actions["team_id"].reset[0]
                firstteam_dict[action.game_id] = games_df["home_team_id"]
            firstteam = firstteam_dict[action.game_id]
            
            team = game_actions.loc[(action.game_id, action.action_id)]["team_id"]
            if team == firstteam:
                output[action.game_id][action.action_id] = predictions[i][0].detach().numpy()
            else:
                output[action.game_id][action.action_id] = np.flip(predictions[i][0].detach().numpy())
        return dict(output)
